...
| 6.8 Recursion; linear & binary search; bubble/insertion/selection/shell sort | Tutorial | Programming exercises | everlearning | `.../Tutorial_06_Finding_Things.ipynb` (search), `.../Tutorial_07_Putting_Things_in_Order.ipynb` (sort), `.../NB1_How_Computers_Think.ipynb`, `.../NB1_The_Number_Detective.ipynb`, `LearningOutcomes/MIT/MIT-6.8_Recursion-Fibonacci-and-Big-O.ipynb` (pulled in from a 2026-08-01 upload) | same files | Linear/binary search and bubble/insertion/selection sort are solidly required. Recursion now has dedicated required-quality content (naive/memoized/iterative Fibonacci with Big-O comparison) usable by either track — **partially resolved this pass**. **Still open:** shell sort remains an optional extension in the Boring track only, and whoever sets the assessment bar should confirm recursion is core-required in both tracks' actual grading, not just available as a tutorial — see `SUGGESTION_MIT-Verify-Minor-Gaps.md` |
