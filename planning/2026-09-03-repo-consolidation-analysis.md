...
The first phase of section 6 was started the same evening as pull requests, and then narrowed on Josh's instruction: nothing is to be deleted from the other repositories, and the good versions are to be gathered in `dewadaba` instead. The three pull requests that removed files (Mathematics, 2plus1Coding, WADB_Tutorials) were closed unmerged. What remains open: the four teaching-module descriptors copied into dewlab's `planning/curriculum/descriptors/` (dewlab #116), this repository's README saying it is an archive of sources (#6), and dewadaba #2, which assembles the Web Authoring and Databases site from WADB_Tutorials, HTML-CSS-SQL-JS, the SQL quiz and the SQLite notebook, with a new hub page that sends students to the `web` starter first.
